				...
				<nav class="navbar navbar-expand">
					...
					<div class="user-box dropdown">
						...
						<ul class="dropdown-menu dropdown-menu-end">
							<li><a class="dropdown-item" href="ad_absen_pg.php?logout"><i class='bx bx-log-out-circle'></i><span>Logout</span></a>
							</li>
						</ul>
					</div>
				</nav>
		...
		<div class="page-wrapper">
			<div class="page-content">
				...
				<h6 class="mb-0 text-uppercase">
					<?php
						if (isset($_GET['i'])) {
							$sql = "SELECT nama_pgw FROM pegawai WHERE nip = '".$_GET['i']."';";
							$nama = mysqli_fetch_assoc(mysqli_query(koneksi(),$sql))['nama_pgw'];
							echo "Data Absensi <b>".$nama."</b>";
						} else {
							echo "Data Absensi Pegawai";
						}
					?>
				</h6>
				<hr/>
					...
					<div class="card-body">
						<div class="table-responsive">
							<table <?php if (isset($_GET['i'])) {echo "id='tbnobutton'";} else {echo "id='tbnobutton'";} ?> class="table table-striped table-bordered">
									...
									<?php
										if (isset($_GET['i'])) {
											detailabsenpg($_GET['i']);
										} else {
											if (isset($_GET['month'])) {
												$tahun = substr($_GET['month'],0,4);
												$bulan = substr($_GET['month'],5,2);
												tampilpg($tahun,$bulan);
											} else {
												tampilpg(date('Y'),date('m'));
											}
										}
									?>
