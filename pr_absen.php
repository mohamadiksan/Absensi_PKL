<?php 
	...
	if (isset($_POST['absen-pagi'])) {
		...
		prabsenpagi($_SESSION['id'],date('Y-m-d H:i:s'),$kegiatan,$_GET['type']);
	}
	if (isset($_POST['absen-pulang'])) {
		prabsenpulang($_POST['id']);
	}
?>
<!doctype html>
<html lang="en">
...
<?php
	if (isset($_GET['type'])) {
		if ($_GET['type'] == 'wfh') {
			echo "<body onload='startTime();wfh();'>";
		} else {
			echo "<body onload='startTime();getLocation();'>";		
		}
	} else {
	   $sql = "SELECT * FROM `absen_prakerin` WHERE id_prakerin = '".$_SESSION['id']."' AND absen_prakerin.tgl_absen BETWEEN '".date('Y-m-d')." 00:00:00.00' AND '".date('Y-m-d')." 23:59:59.999';";
			$query = mysqli_query(koneksi(),$sql);
			if(mysqli_num_rows($query) > 0){
			    if(mysqli_fetch_assoc($query)['keterangan'] != "WFO"){
	    		    echo "<body onload='startTime();getLocation();'>";
	    		} else {
	    		    echo "<body onload='startTime();wfh();'>";    
	    		}
	...
	<div class="wrapper">
				...
				<nav class="navbar navbar-expand">
					...
					<div class="user-box dropdown">
						...
						<ul class="dropdown-menu dropdown-menu-end">
							<li><a class="dropdown-item" href="pr_absen.php?logout"><i class='bx bx-log-out-circle'></i><span>Logout</span></a>
							</li>
						</ul>
					</div>
				</nav>
		...
		<div class="page-wrapper">
			<div class="page-content">
				...
				<!--end breadcrumb-->
				<h6 class="mb-0 text-uppercase">ABSENSI PRAKERIN INOVINDO</h6>
				<hr/>
        ...
        <?php 
			...
			<?php } else {
        if (empty($_GET['type']) && mysqli_num_rows($query) == 0) { ?>
																	        	<div class="col" id="option" style="display: block;">
																							<div class="text-center text-light d-flex justify-content-center radius-10  mb-3"> 
																								<a href='pr_absen.php?type=wfo'><button type='button' class='btn btn-dark radius-10 me-2'><i class='bx bx-buildings'></i>Absen WFO</button></a>
																								<a href='pr_absen.php?type=wfh'><button type='button' class='btn btn-dark radius-10 me-2'><i class='bx bx-home'></i>Absen WFH</button></a>
																							</div>
																						</div>		
        <?php	} else { ?>
																										...
																										<div class="text-center">
																		                  						...
																		                  						<div class="mb-2">
																		                  							<form action="#" method="POST">
																		                  								<input type="text" class="form-control mb-2 mx-auto" style="width:30%;" name="kegiatan" placeholder="Rancangan Kerja Hari Ini">
																						...
																						<div class="card radius-10">
																							<div class="card-body p-0 pb-3">
																								...
																								<div class="text-center">
																                  						...
																                  						<div class="mb-1">
																                    						<form action="#" method="POST">
																          												<input type="hidden" name="id" <?php echo "value='".$data['id_absen_prakerin']."'"; ?>>
					...
					<?php } elseif($data['keterangan'] == "Masuk") { ?>
																						<div class="col" id="form-kerja-pulang" style="display: none;">
																						<div class="text-center text-light d-flex justify-content-center radius-10 bg-success mb-3"> 
																							<i class="bx bx-check p-2"></i>
																        						<p class="p-2 mb-0">Anda Telah Melakukan Absen</p>
		...
		function startTime() {
		  ...
		  if (!getLocation()) {
		  	if (h >= 9 && h < 10 && m <= 30) {
			  	...
			  	document.getElementById('form-kerja-pulang').style.cssText = 'display:none !important';
			  	document.getElementById('form-kerja-masuk').style.cssText = 'display:none !important';
			  	document.getElementById('form-lembur-mulai').style.cssText = 'display:block !important';
			  } else {
			  	document.getElementById('diluar-jam').style.cssText = 'display:block !important';
			  	document.getElementById('form-kerja-masuk').style.cssText = 'display:none !important';
			  	document.getElementById('form-kerja-pulang').style.cssText = 'display:none !important';
			  	document.getElementById('form-lembur-mulai').style.cssText = 'display:none !important';
			  }
		  } else {
	  		document.getElementById('diluar-lokasi').style.cssText = 'display:block !important';
		  	document.getElementById('form-kerja-pulang').style.cssText = 'display:none !important';
