<?php
	...
	if (isset($_POST['simpanpenilaian'])) {
		for ($i=1; $i < (count($_POST)/4); $i++) {
			...
			if (isset($aspek) && $nilai != 0) {
				penilaian($aspek,$cat,$nilai,$ket,$id_prakerin,koneksi());	
				//echo $aspek.$cat.$nilai.$ket.$id_prakerin."<br>";
			}
		}
		echo "<script type='text/javascript'>window.location = 'ad_penilaian.php';</script>";
	}
	/*if (isset($_POST['simpanpenilaian'])) {
		var_dump
	}*/
	...
	<!--wrapper-->
				...
				<nav class="navbar navbar-expand">
					...
					<div class="user-box dropdown">
						...
						<ul class="dropdown-menu dropdown-menu-end">
							<li><a class="dropdown-item" href="ad_tambahpenilaian.php?logout"><i class='bx bx-log-out-circle'></i><span>Logout</span></a>
							</li>
						</ul>
					</div>
				</nav>
		...
		<div class="page-wrapper">
			...
			<div class="d-flex justify-content-between flex-column">
				...
				<select class="form-control" style="width: 300px;" name="det">
					<?php
						optionasal();
					?>
				</select>
				<button type="submit" name="copy" class="btn btn-primary px-1 mt-1 radius-10" style="width: 120px;"><i class='bx bx-info-circle'></i>Tampilkan</button>
				</form>
			</div>
			<br>
			...
			<div class="card radius-10">
				<div class="mb-3">
					<div class="card-body" id="card-sikap">
						<?php 
							if (isset($_POST['det'])) {
										$sql = "SELECT hasil_prakerin.aspek,hasil_prakerin.category FROM hasil_prakerin JOIN prakerin ON prakerin.id_prakerin = hasil_prakerin.id_prakerin WHERE hasil_prakerin.category = 'Sikap' AND prakerin.asal_sekolah = '".$_POST['det']."';";
									...
									<div class='col-sm-9' style='display:flex; justify-content:space-around; width:100%'>
										<input type='hidden' name='cat".$num."' value='Sikap'>
										<input type='text' class='form-control' id='form' style='margin-right: 20px;' placeholder='Aspek' name='aspek".$num."' value='".$row['aspek']."'>
										<input type='text' class='form-control' id='form' style='width: 150px; margin-right: 20px; ' placeholder='Nilai' name='nilai".$num."'>
										<input type='text' class='form-control' id='form' placeholder='Keterangan' name='ket".$num."'>
									</div>
								</div>";
								$num++;
									}
				...
				function addPengetahuan() {
					...
					var cln = itm[0].cloneNode(true);
				  	...
				  	for(var i = 1;i < itm.length;i++){
				  		itm[i].children[0].children[0].name = "cat"+(i+sikap.length+1);
				  		itm[i].children[0].children[1].name = "aspek"+(i+sikap.length+1);
				  		itm[i].children[0].children[2].name = "nilai"+(i+sikap.length+1);
				  		itm[i].children[0].children[3].name = "ket"+(i+sikap.length+1);
					}
					keterampilanUpdate();
				}
				...
				<div class="mb-3">		
					<div class="card-body" id="card-pengetahuan">
						...
						<?php 
							...
							if (isset($_POST['det'])) {
								$sql = "SELECT hasil_prakerin.aspek,hasil_prakerin.category FROM hasil_prakerin JOIN prakerin ON prakerin.id_prakerin = hasil_prakerin.id_prakerin WHERE hasil_prakerin.category = 'Pengetahuan' AND prakerin.asal_sekolah = '".$_POST['det']."';";
								$data = mysqli_query(koneksi(),$sql);
							foreach ($data as $row) {
										echo "<div class='row mb-3 form-pengetahuan' id='form-pengetahuan'>
									<div class='col-sm-9' style='display:flex; justify-content:space-around; width:100%'>
										<input type='hidden' name='cat".$num."' value='Pengetahuan'>
										<input type='text' class='form-control' id='form' style='margin-right: 20px;' placeholder='Aspek' name='aspek".$num."' value='".$row['aspek']."'>
										<input type='text' class='form-control' id='form' style='width: 150px; margin-right: 20px; ' placeholder='Nilai' name='nilai".$num."'>
										<input type='text' class='form-control' id='form' placeholder='Keterangan' name='ket".$num."'>
									</div>
			...
			<div class="mb-3">
				<h6>KETERAMPILAN</h6><hr/>
						...
						function addKeterampilan() {
							...
							for(var i = 1;i < itm.length;i++){
				  				...
				  				itm[i].children[0].children[1].name = "aspek"+(i+peng.length+sikap.length+1);
				  				itm[i].children[0].children[2].name = "nilai"+(i+peng.length+sikap.length+1);
				  				itm[i].children[0].children[3].name = "ket"+(i+peng.length+sikap.length+1);
							}
				...
				<div class="mb-1">		
					<div class="card-body" id="card-keterampilan">
						...
						<?php 
							if (isset($_POST['det'])) {
								$sql = "SELECT hasil_prakerin.aspek,hasil_prakerin.category FROM hasil_prakerin JOIN prakerin ON prakerin.id_prakerin = hasil_prakerin.id_prakerin WHERE hasil_prakerin.category = 'Keterampilan' AND prakerin.asal_sekolah = '".$_POST['det']."';";
									$data = mysqli_query(koneksi(),$sql);
									foreach ($data as $row) {
										echo "<div class='row mb-3 form-keterampilan' id='form-keterampilan'>
									<div class='col-sm-9' style='display:flex; justify-content:space-around; width:100%'>
										<input type='hidden' name='cat".$num."' value='Keterampilan'>
										<input type='text' class='form-control' id='form' style='margin-right: 20px;' placeholder='Aspek' name='aspek".$num."' value='".$row['aspek']."'>
										<input type='text' class='form-control' id='form' style='width: 150px; margin-right: 20px; ' placeholder='Nilai' name='nilai".$num."'>
										<input type='text' class='form-control' id='form' placeholder='Keterangan' name='ket".$num."'>
									</div>
       ...
       <!--  <div class="card-body" style="overflow-x:scroll;">
          ...
          <div class="col m-3">
			...
			<button type="submit" name="simpanpenilaian" class="btn btn-primary px-2 radius-10 mb-5"><i class='bx bx-save'></i>Simpan</button>
			</div>
		</form>
		<!--end page wrapper -->
		<!--start overlay-->
